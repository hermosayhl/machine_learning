...
namespace pipeline {
    ...
    std::map<const char*, dataset_type> read_datasets(const std::string& dataset_name="mnist", const bool show=false) {
        // 如果是 mnist 数据集
        if(dataset_name == "mnist") {
            ...
            if(not std::filesystem::exists(mnist_cache)) {
                ...
                // =======================================================================
                if(show) {
                    print(train_features[200], train_labels[200]);
                    print(train_features[50000], train_labels[50000]);
                    print(test_features[520], test_labels[520]);
                }
            }
